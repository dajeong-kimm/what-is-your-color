    ...
    <div class="hide-on-small">
      <div class="tool-wrapper">
        ...
        <div class="button-wrapper">
          <!-- 로그인 안된 상태 -->
          <button v-if="!state.isLogin" @click="clickRegister">회원가입</button>
          <button v-if="!state.isLogin" @click="clickLogin">로그인</button>
        </div>
        <div class="button-wrapper">
          <!-- 로그인 된 상태 -->
          <button v-if="state.isLogin" @click="clickHistory">지난 회의 내역</button>
          <button v-if="state.isLogin" @click="clickLogout">로그아웃</button>
        </div>
      </div>
    </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const clickHistory = () => {
      router.push('/history');  // 예시로 '지난 회의 내역' 페이지로 이동
    };

    const clickLogout = () => {
      store.dispatch('accountStore/logoutAction');  // 로그아웃 액션
